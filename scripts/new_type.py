...
Usage:
    ...
    3. Edit templates/<type>.md and .claude/rules/<type>.md
...
def main() -> int:
    ...
    rule = RULES_DIR / f"{spec.get('rules', name)}.md"
    if not rule.exists():
        rule.write_text(
            RULE.format(name=name, folder=spec["folder"], label=spec["label"]),
            encoding="utf-8",
        )
        created.append(str(rule.relative_to(ROOT)))
