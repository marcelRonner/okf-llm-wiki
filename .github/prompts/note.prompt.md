...
nothing to file in `raw/` — just what they said.
...
said. **Append only** — entries are never edited or removed, exactly like anything in `raw/`.
...
`origin: owner` matters — it tells `make lint` not to look for a file in `raw/` that was never
...
For a new page, choose the type with the ladder in `copilot-instructions.md` and say in one line
...
and switch to `/ingest`, which will preserve it in `raw/` properly. The dividing line is whether
