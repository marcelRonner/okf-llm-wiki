...
One source page per item in `content/references/`. It records **what the material said** and **what it changed
...
| | |
|---|---|
| **File** | [`2026-08-01-name.pdf`](../references/2026-08-01-name.pdf) |
...
- `origin: owner` in the frontmatter, which exempts it from the `content/references/` pairing check. Without
...
- Entries are **quoted as said**, not summarised. The owner's words are the primary record here,
  exactly as a PDF's words are in `content/references/`. Your writing goes on the derived page.
- It is **append-only**, like anything in `content/references/`. If an entry turns out to be wrong, add a dated
