...
.github/            GENERATED — only the scoped rules, which Copilot spells differently
...
| `.github/instructions/` | **Generated** from `.claude/rules/`. The only thing that still needs a copy |
...
The exception is path-scoped rules. The `.claude/rules` location Copilot supports is user-profile
only (`~/.claude/rules`), so a *project's* scoped rules have to be restated as
`.github/instructions/*.instructions.md`, where `paths:` is spelled `applyTo:`. That one
translation is generated by `make schema`, and `make build` fails if it has drifted — the same
treatment as every other restatement here. If Copilot ever reads project-level `.claude/rules/`,
that phase and the whole `.github/` directory can go.
...
a cross-tool standard, so these four files need no Copilot translation, where the prompt files they
replaced did.
