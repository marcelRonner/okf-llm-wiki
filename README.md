...
`/ingest`. The assistant reads them, files the original into `content/references/`, writes the pages they
...
### `content/references/` versus `content/sources/`

These folders have different jobs:

| Location | Holds | Can it be edited? |
|---|---|---|
| `content/references/` | The original material as received: a PDF, export, copied note, or other evidence. | **Only its frontmatter.** A Markdown original gets a header once, when filed, so an Open Knowledge Format consumer can read it as part of the bundle. The body below the header stays as received. |
...
├── sources/     one page per item in content/references/, plus monthly owner notes
└── references/  the originals — only ever gains a frontmatter header
...
| `.claude/rules/sources.md` | Source pages, their link to `content/references/`, and the owner-notes pages |
...
loaded rather than all of them on every request.

Almost none of this needs a second copy, because VS Code and Claude Code have converged on the same
files. VS Code reads `AGENTS.md` at the workspace root, and discovers skills from `.github/skills/`,
`.claude/skills/` **and** `.agents/skills/` — so the contract and the four operations are read where
they are written, by both tools, with no mirror at all.

Two things still need writing into `.github/`, and neither is a copy of the instructions:

- **`instructions/`** — the `.claude/rules` location Copilot supports is user-profile only
  (`~/.claude/rules`), so a *project's* scoped rules have to be restated as
  `.github/instructions/*.instructions.md`, where `paths:` is spelled `applyTo:`.
- **`prompts/`** — Copilot matches a skill on its description rather than on a name you type, and
  four typed doors is the whole point of *nothing enters or leaves the wiki except through these
  four*. Each prompt file names its skill and stops — eleven lines, no protocol in it. Type `/note`
  in either tool and the same skill runs.

Both are generated by `make schema`, and `make build` fails if either has drifted — the same
treatment as every other restatement here. If Copilot ever reads project-level `.claude/rules/` and
lets a skill be invoked by name, both phases and the whole `.github/` directory can go.
...
having. Skills win on three counts, which is why the operations live there. A skill is a directory,
so `/ingest` has somewhere to keep a helper script if it ever needs one. A skill loads
progressively — only its `description` sits in context until it runs, so four operations cost four
lines rather than four hundred. And skills are the one format both tools already read: `SKILL.md` is
a cross-tool standard, so the procedure itself is never translated — the prompt files under
`.github/` are eleven-line doors that point back here, not second copies of it.
